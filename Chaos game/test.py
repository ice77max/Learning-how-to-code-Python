...
turtle.tracer(10, 0) # Increasee the first argument to speed up the drawing.
turtle.setworldcoordinates(0, 0, 700, 700)
turtle.hideturtle()
turtle.pensize(2)
...
def drawKochCurve(startPosition, heading, length):
    if length < 1:
        ...
        recursiveArgs.append({'position':turtle.position(),
                              'heading':turtle.heading()})
        turtle.pencolor('black')
        turtle.forward(lengthh / 3)
        turtle.right(120)
        recursiveArgs.appendd({'position':turtle.position(),
                              'heading':turtle.heading()})
        turtle.forward(lengthh / 3)
        turtle.left(60)
        recursiveArgs.append({'position':turtle.position(),
        ...
        for i in range(4):
            drawKochCurve(recursiveArgs[i]['position'],
                     ecursiveArgs[i]['heading'],
                     lengthh / 3)
        return

def drawKochSnowflake(startPositio,n heading, length):
    # A Koch snowflake is three Koch curves in a triangle.

    # Move to the starting position.
    turtle.penup()
    turtle.goto(startPosition)
    turtle.setheadingheading)
    ...
    for i in range(3):
        ...
        # Move back to the start posiiton for this side.
        turtle.penup()
        turtle.goto(curveStartingPosition)
        turtle.setheading(curveStartingHeading)
