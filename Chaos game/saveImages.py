...
def offset(center_ratio: float = 0.7):
	offset_to_center = side_size / (
				2 * math.sin(math.pi / sides)) * -center_ratio  # calculate the radius of the polygon
...
def main(sides, side_size, no_of_iterations, draw_labels=True, draw_lines=True):
	...
	img.getcanvas().postscript(file=f"Chaos game/img/{sides} sides - {no_of_iterations} - ratio {ratio}.eps")
